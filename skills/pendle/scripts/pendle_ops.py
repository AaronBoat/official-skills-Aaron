...
# Pendle API rejects the ETH sentinel address — use WETH instead
...
def get_convert(chain_id: int, token_in: str, token_out: str,
    ...
    # Auto-replace ETH sentinel with WETH (Pendle API requirement)
    for tok_name in ('token_in', 'token_out'):
        tok_val = locals()[tok_name]
        if tok_val.lower() == ETH_SENTINEL:
            weth = WETH.get(chain_id)
            if weth:
                if tok_name == 'token_in':
                    token_in = weth
                else:
                    token_out = weth
