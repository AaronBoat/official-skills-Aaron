...
def get_positions(user_address: str, chain_id: int = 1):
    ...
    query($userAddress: String!, $chainId: Int) {
      vaultPositions(where: { userAddress: $userAddress, chainId: $chainId }) {
        items {
    ...
    """
    result = gql_query(query, {"userAddress": user_address.lower(), "chainId": chain_id})
    ...
    for p in positions:
        ...
        usd = float(p.get("supplyAssetsUsd", 0))
